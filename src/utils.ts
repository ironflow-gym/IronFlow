...
// Index 0–4 map to the five standard levels. -1 = below the first threshold (Building).
const STRENGTH_LEVEL_LABELS = ['Building', 'Developing', 'Established', 'Forged', 'Elite'];
const STRENGTH_LEVEL_LABEL_DEVELOPING = 'Foundations';
...
export function getRelativeStrength(
  ...
  return Object.entries(bests).map(([key, { e1rm, name, date }]) => {
    ...
    // -1 = below the first (Building) threshold
    let levelIndex = -1;
