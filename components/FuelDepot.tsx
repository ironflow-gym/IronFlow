...
const sanitizeProfileUpdate = (baseProfile: FuelProfile, updatedProfile?: Partial<FuelProfile>): FuelProfile => {
  ...
  const defaultRatioForGoal = defaultProteinRatioByGoal(goal);
  const targetProteinRatio = typeof updatedProfile.targetProteinRatio === 'number' && Number.isFinite(updatedProfile.targetProteinRatio)
    ? Math.min(3.0, Math.max(0.6, updatedProfile.targetProteinRatio))
    : (updatedProfile.goal && updatedProfile.goal !== baseProfile.goal ? defaultRatioForGoal : baseProfile.targetProteinRatio);
...
const normalizePreferences = (preferences: string[] = []): string[] => {
  return Array.from(new Set(preferences.map(p => p.toLowerCase().trim()).filter(Boolean)));
};

const deriveMacroRatios = (goal: FuelProfile['goal'], preferences: string[]) => {
  const pref = normalizePreferences(preferences);
  const hasKeto = pref.some(p => p.includes('keto') || p.includes('low carb'));
  const hasHighProtein = pref.some(p => p.includes('high protein'));
  ...
  if (hasKeto) {
    ...
    carbShare = Math.max(0.35, carbShare - 0.08);
    fatShare = 1 - carbShare;
  }

  let proteinAdjustment = 1.0;
  if (pref.some(p => p.includes('vegan'))) proteinAdjustment = 1.15;
  else if (pref.some(p => p.includes('vegetarian'))) proteinAdjustment = 1.08;

  return { carbShare, fatShare, proteinAdjustment };
};

const defaultProteinRatioByGoal = (goal: FuelProfile['goal']): number => {
  if (goal === 'Build Muscle') return 1.8;
  if (goal === 'Lose Fat') return 1.6;
  return 1.2;
};
...
const FuelDepot: React.FC<FuelDepotProps> = ({ history, profile, onSaveFuel, onSaveProfile, biometricHistory, aiService, userSettings }) => {
  ...
  const macroConfig = useMemo(() => deriveMacroRatios(profile.goal, profile.preferences), [profile.goal, profile.preferences]);
  const baseProteinRatio = typeof profile.targetProteinRatio === 'number' && Number.isFinite(profile.targetProteinRatio)
    ? profile.targetProteinRatio
    : defaultProteinRatioByGoal(profile.goal);
  const effectiveProteinRatio = Math.min(3.0, Math.max(0.6, baseProteinRatio * macroConfig.proteinAdjustment));

  const targetProtein = Number((latestWeight * effectiveProteinRatio * multiplier).toFixed(1));
  const proteinCalories = targetProtein * 4;
  const remainingCalories = Math.max(0, estimatedTDEE - proteinCalories);
  const targetCarbs = Number(((remainingCalories * macroConfig.carbShare) / 4).toFixed(1));
  const targetFats = Number(((remainingCalories * macroConfig.fatShare) / 9).toFixed(1));

  const dailySummaries = useMemo(() => {
    const grouped: Record<string, { logs: FuelLog[], totals: { calories: number, protein: number, carbs: number, fats: number } }> = {};
    
    // Sort all history by date descending
