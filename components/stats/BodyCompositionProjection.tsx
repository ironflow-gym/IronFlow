...
const BodyCompositionProjection: React.FC<Props> = ({ biometricHistory, userSettings }) => {
  ...
  const toDisplay = (kg: number) => userSettings.units === 'imperial' ? Math.round(kg * 2.20462 * 10) / 10 : kg;
  ...
  // Merge history + projection into one chart series
  const chartData = useMemo(() => {
    ...
    // Bridge: last historical point doubles as first proj point
    if (hist.length > 0 && projPoints.length > 0) {
    ...
    <div className="flex flex-col gap-3 h-full">
              ...
              <>
                ...
                <div className="absolute left-0 top-6 z-50 w-72 bg-slate-900 border border-slate-700 rounded-2xl p-4 shadow-2xl space-y-2">
                  ...
                  <p className="text-[10px] text-slate-400 leading-relaxed">
                    Uses linear regression on your logged bodyweight (and body fat % if available) to project where your current trend puts you in 90 days.
                  </p>
                  <p className="text-[10px] text-amber-400/80 leading-relaxed">
                    The dashed projection assumes your current trend continues unchanged. It is a planning tool, not a prediction. More entries = more accurate projection.
                  </p>
                </div>
              </>
            )}
      ...
      <div className="flex-1 min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 4, right: 8, left: -8, bottom: 0 }}>
            ...
            <XAxis dataKey="date" tick={{ fill: '#64748b', fontSize: 9, fontWeight: 700 }} tickLine={false} axisLine={false}
              interval="preserveStartEnd" />
            <YAxis tick={{ fill: '#64748b', fontSize: 9, fontWeight: 700 }} tickLine={false} axisLine={false}
              tickFormatter={v => `${v}${weightUnit}`} width={40} />
            <Tooltip
              contentStyle={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, fontSize: 11, fontWeight: 700 }}
              labelStyle={{ color: '#94a3b8' }}
              formatter={(val: number, name: string) => {
                const labels: Record<string, string> = {
                  weight: `Weight (${weightUnit})`, projWeight: `Projected weight (${weightUnit})`,
                  lean: `Lean mass (${weightUnit})`, projLean: `Projected lean (${weightUnit})`,
                  fat: `Fat mass (${weightUnit})`, projFat: `Projected fat (${weightUnit})`,
                };
                return [val, labels[name] ?? name];
              }}
            />
            {/* Actual lines */}
            <Line dataKey="weight" stroke="#94a3b8" strokeWidth={2} dot={{ r: 2.5, fill: '#94a3b8', strokeWidth: 0 }} connectNulls name="weight" />
            {hasBodyFat && <Line dataKey="lean" stroke="#10b981" strokeWidth={2} dot={{ r: 2, fill: '#10b981', strokeWidth: 0 }} connectNulls name="lean" />}
            {hasBodyFat && <Line dataKey="fat" stroke="#f97316" strokeWidth={2} dot={{ r: 2, fill: '#f97316', strokeWidth: 0 }} connectNulls name="fat" />}
            {/* Projection lines (dashed) */}
            {proj.hasProjection && (
              <>
                <Line dataKey="projWeight" stroke="#94a3b8" strokeWidth={1.5} strokeDasharray="5 4" dot={false} connectNulls name="projWeight" />
                {hasBodyFat && <Line dataKey="projLean" stroke="#10b981" strokeWidth={1.5} strokeDasharray="5 4" dot={false} connectNulls name="projLean" />}
                {hasBodyFat && <Line dataKey="projFat" stroke="#f97316" strokeWidth={1.5} strokeDasharray="5 4" dot={false} connectNulls name="projFat" />}
              </>
            )}
          </ComposedChart>
        </ResponsiveContainer>
      ...
      <div className="flex items-center gap-4 shrink-0">
        <span className="text-[9px] font-black text-slate-500 flex items-center gap-1.5">
          <span className="w-4 h-px bg-slate-400 inline-block" /> Actual
        </span>
        {proj.hasProjection && (
          <span className="text-[9px] font-black text-slate-500 flex items-center gap-1.5">
            <span className="w-4 h-px border-t border-dashed border-slate-400 inline-block" /> 90-day projection
          </span>
        )}
      </div>
    </div>
