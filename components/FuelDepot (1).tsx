...
const FuelDepot: React.FC<FuelDepotProps> = ({ history, profile, onSaveFuel, onSaveProfile, biometricHistory, aiService, userSettings }) => {
  ...
  const estimatedTDEE = useMemo(() => {
    ...
    // targetMultiplier scales the activity PAL factor (activity fine-tune),
    // then the fixed surplus/deficit is applied on top. This preserves the
    // full deficit/surplus regardless of multiplier value.
    // e.g. mult=1.2 means "I'm ~20% more active than the goal default" —
    // the -500 kcal deficit is still the full -500 after scaling.
    let actMult = 1.375;
    if (profile.goal === 'Build Muscle') actMult = 1.55;
    if (profile.goal === 'Lose Fat') actMult = 1.4;
    const scaledTDEE = bmr * actMult * multiplier;
    const adjusted = profile.goal === 'Build Muscle' ? scaledTDEE + 300 : (profile.goal === 'Lose Fat' ? scaledTDEE - 500 : scaledTDEE);
    const result = Number(adjusted.toFixed(1));
