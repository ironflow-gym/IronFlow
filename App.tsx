...
const App: React.FC = () => {
        ...
        if (initialSettings.ironSyncConnected && navigator.onLine) {
          ...
          try {
            ...
            if (cloudMirror && cloudMirror.lastUpdated > (initialSettings.lastCloudSync || 0)) {
              ...
              initialSettings = mergedSettingsWithDefault(refreshedSettings);
              setUserSettings(initialSettings);
            }
  ...
  const refreshLocalState = async () => {
    const [
      ...
      storedActiveSession, storedSummaries
    ] = await Promise.all([
  ...
  const triggerSync = async () => {
    if (!userSettings.ironSyncConnected || !isOnline || !ironSync.hasValidToken()) {
      if (userSettings.ironSyncConnected) setSyncStatus('pending');
      return;
    }
    setSyncStatus('transmitting');
    try {
  ...
  useEffect(() => { if (isHydrated) { if (activeSession) storage.set('ironflow_active_session', activeSession); else storage.remove('ironflow_active_session'); } }, [activeSession, isHydrated]);
  ...
  const updateTemplate = (updated: WorkoutTemplate) => { setSavedTemplates(prev => prev.map(t => String(t.id) === String(updated.id) ? updated : t)); setEditingTemplate(null); triggerSync(); };
    ...
    <div className="min-h-screen pb-24 bg-slate-950 text-slate-100 flex flex-col items-center">
      <header className="w-full max-w-2xl px-6 py-8 flex justify-between items-center relative z-[60]">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2"><div className={`p-1.5 rounded-full border transition-all duration-500 ${getSyncColorClass(syncStatus)}`} title={`IronSync Status: ${syncStatus}`}><Cloud size={14} /></div><div className={`p-1.5 rounded-full border transition-colors ${isOnline ? 'border-emerald-500/30 text-emerald-400 bg-emerald-500/10' : 'border-rose-500/30 text-rose-400 bg-rose-500/10'}`}>{isOnline ? <Wifi size={14} /> : <WifiOff size={14} />}</div></div>
        </div>
      ...
      {isSettingsOpen && <SettingsModal settings={userSettings} syncStatus={syncStatus} onSave={(s) => { setUserSettings(s); setIsSettingsOpen(false); triggerSync(); }} onClose={() => setIsSettingsOpen(false)} aiService={aiService.current} onUpdateCustomLibrary={setCustomLibrary} onRefreshState={refreshLocalState} />}
