...
export class GeminiService {
  ...
  async parseFuelPrompt(prompt: string, profile: FuelProfile, pantryContext?: FoodItem[]): Promise<{ logs: FuelLog[], updatedProfile?: FuelProfile }> {
    ...
    try {
      const response = await this.ai.models.generateContent({
        model: MODEL_FLASH,
        contents: `Date: ${now}. Goal: ${profile.goal}. Preferences: ${JSON.stringify(profile.preferences || [])}. Protein target: ${profile.targetProteinRatio}g/kg. ${pantryText}\nUser input: "${prompt}"`,
        config: {
          systemInstruction: "You are a sports nutritionist. Do two things: (1) Extract food items and macros from the user input — prioritise exact pantry matches over estimates. Respect dietary preferences from the provided profile (e.g. vegan, vegetarian, lactose intolerant, gluten free, keto) and avoid returning foods that violate explicit restrictions. Confidence: 1.0=exact pantry match, 0.8=well-known product, 0.5=estimated. (2) If the input includes goal-setting or dietary preference statements (e.g. 'I want to lose fat', 'I am vegetarian', 'I'm bulking', 'high protein', 'cut calories'), return updatedProfile with the appropriate goal and/or preferences. When goal changes and no explicit protein ratio is given, set targetProteinRatio to: Build Muscle=1.8, Lose Fat=1.6, Maintenance=1.2 g/kg. If no goal/preference information is present, omit updatedProfile entirely.",
          responseMimeType: "application/json",
          responseSchema: {
            type: Type.OBJECT,
            properties: {
